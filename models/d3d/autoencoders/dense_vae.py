...
from models.d3d.norm import GroupNorm32, ChannelLayerNorm32
from models.d3d.spatial import pixel_shuffle_3d
from models.d3d.utils import zero_module, convert_module_to_f16, convert_module_to_f32
from .distributions import DiagonalGaussianDistribution
from models.d3d.dtypes import DenseDecoderInputs
...
class DenseShapeVAE(nn.Module):
    ...
    def decode_mesh(self, inputs: DenseDecoderInputs):
        x = self.decoder(inputs.latents)
        if inputs.return_index:
            outputs = []
            for i in range(len(x)):
                occ = x[i].sigmoid()
                occ = (occ >= inputs.mc_threshold).float().squeeze(0)
                index = occ.unsqueeze(0).nonzero()
                outputs.append(index)
        else:
            outputs = self.dense2mesh(x, voxel_resolution=inputs.voxel_resolution, mc_threshold=inputs.mc_threshold)
