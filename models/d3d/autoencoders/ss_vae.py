...
from models.d3d.dtypes import SparseDecoderInputs
...
class SparseSDFVAE(nn.Module):
    ...
    def decode_mesh(self, inputs: SparseDecoderInputs):
        voxel_resolution = int(inputs.voxel_resolution / inputs.factor)
        reconst_x = self.decoder(inputs.latents, factor=inputs.factor, return_feat=inputs.return_feat)
        if inputs.return_feat:
            return reconst_x
        outputs = self.sparse2mesh(reconst_x, voxel_resolution=voxel_resolution, mc_threshold=inputs.mc_threshold)
