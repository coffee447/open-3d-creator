...
import models.d3d.sparse as sp
...
class Voxel_RefinerXL(nn.Module):
    ...
    def run(self,
        ...
        sdfs = torch.stack(sdfs, dim=0).to(dtype).cpu()

        # feats = torch.zeros((batch_size, sparse_feat.shape[-1], res, res, res), device=device, dtype=dtype)
        feats = torch.zeros((batch_size, sparse_feat.shape[-1], res, res, res), dtype=dtype)
        feats[sparse_index[...,0],:,sparse_index[...,1],sparse_index[...,2],sparse_index[...,3]] = sparse_feat.cpu()
        ...
        for i in range(step):
            for j in range(step):
                for k in tqdm(range(step)):
                    ...
                    output = F.tanh(final_feat).cpu()
                    patchs.append(output)
                    torch.cuda.empty_cache()
        weights = torch.linspace(0, 1, steps=32, dtype=dtype)
        lines=[]
        for i in range(9):
            out1 = patchs[i * 3]
            out2 = patchs[i * 3 + 1]
            out3 = patchs[i * 3 + 2]
            line = torch.ones([N, 1, 192, 192, res], dtype=dtype) * 2
            line[:, :, :, :, :160] = out1[:, :, :, :, :160]
        ...
        for i in range(3):
            ...
            line2 = lines[i*3+1]
            line3 = lines[i*3+2]
            layer = torch.ones([N,1,192,res,res], dtype=dtype) * 2
            layer[:,:,:,:160] = line1[:,:,:,:160]
